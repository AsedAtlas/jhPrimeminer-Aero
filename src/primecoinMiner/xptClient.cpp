 ...
 bool xptClient_decodeBase58(char* base58Input, sint32 inputLength, uint8* dataOut, sint32* dataOutLength)
 {
   ...
   for(sint32 i=0; i<inputLength; i++)
   {
     if( base58Input[i] >= sizeof(base58Decode)/sizeof(base58Decode[0]) )
       return false;
     char digit = base58Decode[base58Input[i]];
     if( digit == -1 )
       return false;
...
//	 uint32 usernameLength = min(127, (uint32)strlen(xptClient->username)); //unused
//   uint32 passwordLength = min(127, (uint32)strlen(xptClient->password)); //unused
	// build the packet
